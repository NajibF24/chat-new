...
function AdminDashboard({ user, handleLogout }) {
  ...
  // ── Avatar picker state ──────────────────────────────────────
  const [avatarPickerBot, setAvatarPickerBot] = useState(null); // bot yang sedang ganti avatar
  ...
  const handleExport = async () => {
    try {
      ...
      if (exportFilter) { const [year, month] = exportFilter.split('-'); url += `?year=${year}&month=${month}`; }
      const response = await axios.get(url, { responseType: 'blob' });
      const urlObj = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = urlObj;
      link.setAttribute('download', exportFilter ? `chat-logs-${exportFilter}.csv` : `chat-logs-all-${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const handleAvatarSaved = (updatedBot) => {
    // Update bot di list tanpa refetch semua
    setBots(prev => prev.map(b => b._id === updatedBot._id ? updatedBot : b));
    // Jika bot yang sedang di-edit, update juga botForm avatar
    if (editingBot?._id === updatedBot._id) {
      setEditingBot(updatedBot);
      setBotForm(prev => ({ ...prev, avatar: updatedBot.avatar }));
    }
  };
  ...
  const lineChartData = {
    ...
    datasets: [{
      ...
      borderColor: '#007857', backgroundColor: 'rgba(0, 120, 87, 0.15)',
      tension: 0.3, fill: true, pointBackgroundColor: '#004E36'
    }]
  };
  ...
  const pieChartData = {
    labels: botLabels,
    datasets: [{ data: stats?.botPopularity?.map(b => b.count) || [], backgroundColor: pieColors.slice(0, botLabels.length), borderWidth: 2, borderColor: '#ffffff' }]
  };
    ...
    <div className="min-h-screen bg-steel-lightest/50 text-gray-800 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-steel-light/30 overflow-hidden">
              ...
              <table className="w-full text-sm text-left text-gray-800">
                <thead className="bg-steel-lightest text-steel uppercase text-xs"><tr><th className="px-6 py-3">Rank</th><th className="px-6 py-3">Username</th><th className="px-6 py-3">Email</th><th className="px-6 py-3 text-right">Messages</th></tr></thead>
                <tbody className="divide-y divide-steel-light/30">
                  {stats.topUsers?.length > 0 ? stats.topUsers.map((u, idx) => (
                    <tr key={idx} className="hover:bg-steel-lightest/50"><td className="px-6 py-3 font-bold text-steel">#{idx+1}</td><td className="px-6 py-3 font-semibold">{u.username}</td><td className="px-6 py-3 text-steel-light">{u.email||'-'}</td><td className="px-6 py-3 text-right font-mono text-primary font-bold">{u.count}</td></tr>
        ...
        {activeTab === 'users' && (
          <div className="bg-white rounded-lg shadow-sm border border-steel-light/30 overflow-hidden">
            ...
            <table className="w-full text-sm text-left text-gray-800">
              <thead className="bg-steel-lightest text-steel uppercase text-xs"><tr><th className="px-6 py-3">User</th><th className="px-6 py-3">Role</th><th className="px-6 py-3">Bots Access</th><th className="px-6 py-3 text-right">Actions</th></tr></thead>
              <tbody className="divide-y divide-steel-light/30">
                {users.map(u => (
                  <tr key={u._id} className="hover:bg-steel-lightest/50">
                    ...
                    <td className="px-6 py-3">{u.assignedBots?.length || 0} assigned</td>
                    <td className="px-6 py-3 text-right"><button onClick={() => handleEditUser(u)} className="text-primary hover:text-primary-dark font-bold hover:underline">Edit</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-steel-light/30 overflow-hidden flex flex-col h-[700px]">
            <div className="px-6 py-4 border-b border-steel-light/30 flex justify-between items-center bg-steel-lightest/50">
              ...
              <div className="flex items-center gap-2">
                <input type="month" value={exportFilter} onChange={(e) => setExportFilter(e.target.value)} className="bg-white border border-steel-light/50 rounded text-sm px-3 py-1.5 outline-none focus:border-primary-dark" />
                <button onClick={handleExport} className="px-4 py-2 bg-primary text-white text-sm font-bold rounded flex items-center gap-2 hover:bg-primary-dark transition-colors shadow-sm">⬇ Export CSV</button>
              </div>
            </div>
            <div className="flex-1 overflow-auto">
              <table className="w-full text-sm text-left text-gray-800">
                <thead className="bg-steel-lightest text-steel uppercase text-xs sticky top-0"><tr><th className="px-6 py-3">Time</th><th className="px-6 py-3">User</th><th className="px-6 py-3">Bot</th><th className="px-6 py-3">Message</th></tr></thead>
                <tbody className="divide-y divide-steel-light/30">
                  {chatLogs.map(log => (
                    <tr key={log._id} className="hover:bg-steel-lightest/50">
                      ...
                      <td className="px-6 py-3 font-medium">{log.userId?.username||'Unknown'}</td>
                      <td className="px-6 py-3 text-primary font-bold">{log.botId?.name||'System'}</td>
                      <td className="px-6 py-3 truncate max-w-xs text-steel" title={log.content}>{log.content||(log.attachedFiles?.length?'📎 Attachment':'-')}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {Array.isArray(bots) && bots.map(bot => (
              <div key={bot._id} className="bg-white rounded-lg shadow-sm border border-steel-light/30 p-6 hover:shadow-md transition-all">
                ...
                {/* ── Avatar + nama + CONFIG button ── */}
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    {/* ✅ Avatar dengan hover → buka picker */}
                    <div className="relative group/avatar cursor-pointer" onClick={() => setAvatarPickerBot(bot)}>
                    ...
                    <div>
                      <h3 className="font-bold text-lg text-primary-dark leading-tight">{bot.name}</h3>
                      <p className="text-xs text-steel-light mt-0.5">{bot.description?.substring(0, 40) || 'No description'}</p>
                    </div>
                  </div>
                  <button onClick={() => handleEditBot(bot)} className="text-steel hover:text-primary-dark bg-steel-lightest hover:bg-steel-light/30 px-3 py-1 rounded text-xs font-bold transition-colors flex-shrink-0">CONFIG</button>
                </div>

                {/* Tags */}
                <div className="flex flex-wrap gap-2 text-[10px] uppercase font-bold tracking-wider mt-3">
                ...
                {/* Tombol ganti avatar di bawah */}
                <button
                  onClick={() => setAvatarPickerBot(bot)}
                  className="mt-4 w-full py-1.5 text-[11px] font-bold text-steel hover:text-indigo-600 hover:bg-indigo-50 rounded-lg border border-steel-light/30 hover:border-indigo-200 transition-all"
                >
                  🎨 Ganti Avatar
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div className="flex items-center gap-4 p-4 bg-steel-lightest/50 rounded-xl border border-steel-light/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-steel-light mt-0.5">Upload gambar, pilih emoji, atau pilih icon</p>
                  {editingBot ? (
                    <button type="button" onClick={() => setAvatarPickerBot(editingBot)}
                      className="mt-2 px-3 py-1.5 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors font-bold">
                      🎨 Ganti Avatar
                    </button>
                  ) : (
                    <p className="text-xs text-steel mt-2 italic">Simpan bot dulu, lalu ganti avatar dari halaman Bots.</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="border border-steel-light/30 rounded-lg p-4 bg-steel-lightest/50">
                  ...
                  {botForm.smartsheetConfig.enabled && (
                    <div className="space-y-2">
                      <input placeholder="Sheet ID (Contoh: 3743772018954116)" className="w-full bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.smartsheetConfig.sheetId} onChange={e=>setBotForm({...botForm, smartsheetConfig:{...botForm.smartsheetConfig, sheetId:e.target.value}})} />
                      <input type="password" placeholder="API Key (Optional)" className="w-full bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.smartsheetConfig.apiKey} onChange={e=>setBotForm({...botForm, smartsheetConfig:{...botForm.smartsheetConfig, apiKey:e.target.value}})} />
                    </div>
                  )}
                </div>
                ...
                <div className="border border-steel-light/30 rounded-lg p-4 bg-steel-lightest/50">
                  ...
                  {botForm.onedriveConfig.enabled && (
                    <div className="space-y-2">
                      <input placeholder="Folder URL" className="w-full bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.onedriveConfig.folderUrl} onChange={e=>setBotForm({...botForm, onedriveConfig:{...botForm.onedriveConfig, folderUrl:e.target.value}})} />
                      <div className="grid grid-cols-3 gap-2">
                        <input placeholder="Tenant ID" className="bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.onedriveConfig.tenantId} onChange={e=>setBotForm({...botForm, onedriveConfig:{...botForm.onedriveConfig, tenantId:e.target.value}})} />
                        <input placeholder="Client ID" className="bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.onedriveConfig.clientId} onChange={e=>setBotForm({...botForm, onedriveConfig:{...botForm.onedriveConfig, clientId:e.target.value}})} />
                        <input type="password" placeholder="Secret" className="bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.onedriveConfig.clientSecret} onChange={e=>setBotForm({...botForm, onedriveConfig:{...botForm.onedriveConfig, clientSecret:e.target.value}})} />
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="border border-steel-light/30 rounded-lg p-4 bg-steel-lightest/50">
                  ...
                  {botForm.kouventaConfig.enabled && (
                    <div className="space-y-2">
                      <input placeholder="Endpoint URL" className="w-full bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.kouventaConfig.endpoint} onChange={e=>setBotForm({...botForm, kouventaConfig:{...botForm.kouventaConfig, endpoint:e.target.value}})} />
                      <input type="password" placeholder="API Key" className="w-full bg-white border border-steel-light/50 rounded p-2 text-xs text-gray-800 outline-none focus:border-primary" value={botForm.kouventaConfig.apiKey} onChange={e=>setBotForm({...botForm, kouventaConfig:{...botForm.kouventaConfig, apiKey:e.target.value}})} />
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 border border-steel-light/30">
            ...
            <div className="space-y-3">
              ...
              <div className="border border-steel-light/50 p-3 rounded max-h-32 overflow-y-auto bg-steel-lightest/50 custom-scrollbar">
                <p className="text-xs font-bold text-steel mb-2 uppercase">Bot Assignment</p>
                {bots.map(b => (
                  <label key={b._id} className="flex items-center gap-2 mb-1 text-gray-800 text-sm font-medium">
      ...
      {avatarPickerBot && (
        <AvatarPicker
          ...
          onClose={() => setAvatarPickerBot(null)}
        />
      )}
    </div>
  );
...
const StatCard = ({ title, value, icon, color, bg, border }) => (
  <div className={`bg-white p-5 rounded-lg border border-steel-light/30 shadow-sm flex items-center justify-between hover:shadow-md transition-all ${border}`}>
    ...
    <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-2xl ${bg} ${color}`}>{icon}</div>
  </div>
);
